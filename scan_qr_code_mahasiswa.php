...
$mahasiswa_name = $row["nama"];
$mahasiswa_foto = $row["foto"];

// Ambil riwayat absensi mahasiswa berdasarkan NIM
$absensi_result = mysqli_query($koneksi, "SELECT * FROM absensi WHERE nim = '$nim' ORDER BY waktu_absen DESC");
?>
          ...
          <ul class="navbar-nav ml-auto">
            <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <span class="mr-2 d-none d-lg-inline text-gray-600 small"><?= htmlspecialchars($mahasiswa_name); ?></span>
                <img class="img-profile rounded-circle" src="<?= htmlspecialchars($mahasiswa_foto); ?>">
              </a>
                ...
                <tbody>
                  <?php while ($absensi = mysqli_fetch_assoc($absensi_result)): ?>
                    <tr>
                      ...
                      <td><?= htmlspecialchars($absensi['kode_mk']); ?></td>
                      <td><?= htmlspecialchars($absensi['nama_mk']); ?></td>
                      <td><?= htmlspecialchars($absensi['sks']); ?></td>
                      <td><?= htmlspecialchars($absensi['dosen_pengajar']); ?></td>
                    </tr>
                  <?php endwhile; ?>
                </tbody>
              </table>
  ...
  <script>
    ...
    function startScanning() {
      ...
      scanner.start({ facingMode: "environment" }, { fps: 10, qrbox: 250 }, (decodedText) => {
        statusMessage.innerText = `QR Code Terdeteksi: ${decodedText}`;
